...
MoonBit `expf(Float) -> Float` implementation.

The generator evaluates `exp` with MPFR using downward and upward rounding. It
starts at 128 bits and doubles the precision until both interval endpoints
...
MoonBit Core against the same independent reference, and verifies positivity
and monotonicity over the ordered corpus. MPFR, GMP, a C compiler, `pkg-config`,
and `make` are needed only to regenerate or verify the corpus; ordinary builds
and tests stay pure MoonBit.
...
- every argument-reduction cell boundary in the finite result range;
- analytic zero/subnormal, subnormal/normal, and finite/infinity rounding
  transitions;
...
```sh
make -C tools/oracle/expf generate
make -C tools/oracle/expf verify
```

For a larger audit that searches for incorrectly rounded results:

```sh
make -C tools/oracle/expf generate SAMPLES_PER_STRATUM=32768 MAX_ULP=0
```
...
On 2026-08-13, the default corpus contained 17,772 unique inputs generated with
...
producing 103,733 unique inputs. Native debug mode found:

```text
input bits    = 0xc2ba1d30
MPFR result   = 0x00006b6b
actual result = 0x00006b6c
error         = 1 ULP
```

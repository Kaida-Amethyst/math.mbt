...
static void add_fixed_inputs(InputSet *set) {
  static const uint32_t fixed_bits[] = {
      ...
      /* One-ULP witness found by the enlarged deterministic audit. */
      UINT32_C(0xc2ba1d30),
  };
  ...
  for (int value = -105; value <= 90; ++value) {
  ...
  static const uint32_t thresholds[] = {
      UINT32_C(0x39000000), UINT32_C(0x3eb17218), UINT32_C(0x3f851592),
      UINT32_C(0x42aeac50), UINT32_C(0x42b17218), UINT32_C(0x42cff1b5),
  };
...
static void add_analytic_boundaries(InputSet *set) {
  ...
  /* Rounding transition from zero to the minimum binary32 subnormal. */
  mpfr_set_ui_2exp(value, 1, -150, MPFR_RNDN);
  mpfr_log(value, value, MPFR_RNDN);
  add_mpfr_neighborhood(set, value, 32);

  /* Minimum subnormal to twice the minimum subnormal. */
  mpfr_set_ui_2exp(value, 3, -150, MPFR_RNDN);
  mpfr_log(value, value, MPFR_RNDN);
  add_mpfr_neighborhood(set, value, 32);

  /* Maximum subnormal to minimum normal. */
  mpfr_set_ui_2exp(value, 1, -126, MPFR_RNDN);
  mpfr_set_ui_2exp(scratch, 1, -150, MPFR_RNDN);
  mpfr_sub(value, value, scratch, MPFR_RNDN);
  mpfr_log(value, value, MPFR_RNDN);
  add_mpfr_neighborhood(set, value, 32);

  /* Round-to-nearest overflow transition above maximum finite binary32. */
...
static void add_random_inputs(InputSet *set, size_t samples_per_stratum) {
  ...
  for (size_t i = 0; i < samples_per_stratum; ++i) {
    ...
    mpfr_mul_ui(value, fraction, 195, MPFR_RNDN);
    mpfr_sub_ui(value, value, 105, MPFR_RNDN);
    ...
    long k = (long)(xorshift64star(&state) % 279) - 150;
...
static uint32_t oracle_expf(uint32_t input_bits,
  ...
  if (input_bits == F32_EXPONENT_MASK ||
    ...
    *used_precision = INITIAL_ORACLE_PRECISION;
    return input_bits == F32_EXPONENT_MASK ? F32_EXPONENT_MASK : 0;
  }
...
static void emit_tests(const OracleCase *cases, size_t count,
  ...
  printf("// Code generated by tools/oracle/expf/generate.c; DO NOT EDIT.\n");
  printf("// Oracle: MPFR %s, exp, binary32 round-to-nearest ties-to-even.\n",
         mpfr_get_version());
  ...
  for (size_t chunk = 0; chunk < chunk_count; ++chunk) {
    ...
    printf("///|\nfn expf_mpfr_cases_%zu() -> Array[(UInt, UInt)] {\n",
           chunk);
    printf("  [\n");
  ...
  printf("///|\nfn expf_mpfr_case_groups() -> Array[Array[(UInt, UInt)]] {\n");
  printf("  [\n");
  for (size_t chunk = 0; chunk < chunk_count; ++chunk) {
    printf("    expf_mpfr_cases_%zu(),\n", chunk);
  }
  printf("  ]\n}\n\n");

  printf("///|\nfn expf_oracle_ulp_error(expect : Float, actual : Float) -> UInt {\n");
  ...
  printf("///|\ntest \"expf agrees with the MPFR oracle within %u ULP\" {\n",
         options->maximum_ulp);
  printf("  let mut maximum_error = 0U\n");
  printf("  for cases in expf_mpfr_case_groups() {\n");
  ...
  printf("      )\n");
  printf("      let actual = @math.expf(input)\n");
  printf("      let error = expf_oracle_ulp_error(expected, actual)\n");
  printf("      if error > maximum_error {\n");
  printf("        maximum_error = error\n");
  printf("      }\n");
  printf("      if error > %uU {\n", options->maximum_ulp);
  printf("        println(\n");
  printf("          \"expf oracle mismatch: input_bits=\\{input_bits}, expected_bits=\\{expected_bits}, actual_bits=\\{actual.reinterpret_as_uint()}, ulp=\\{error}\",\n");
  printf("        )\n");
  ...
  printf("///|\ntest \"MoonBit Core expf meets the oracle bound over the corpus\" {\n");
  printf("  for cases in expf_mpfr_case_groups() {\n");
  printf("    for pair in cases {\n");
  printf("      let (input_bits, expected_bits) = pair\n");
  printf("      let input = Float::reinterpret_from_int(input_bits.reinterpret_as_int())\n");
  ...
  printf("        expected_bits.reinterpret_as_int(),\n");
  printf("      )\n");
  printf("      let actual = @core_math.expf(input)\n");
  printf("      assert_true(expf_oracle_ulp_error(expected, actual) <= %uU)\n",
         options->maximum_ulp);
  printf("    }\n");
  printf("  }\n");
  printf("}\n\n");

  printf("///|\ntest \"expf range and monotonicity over the oracle corpus\" {\n");
  ...
  printf("      assert_eq(actual.reinterpret_as_uint() >> 31, 0U)\n");
...
int main(int argc, char **argv) {
  ...
  for (size_t i = 0; i < inputs.length; ++i) {
    ...
    cases[i] = (OracleCase){
        ...
        .expected_bits = oracle_expf(inputs.items[i], &used_precision),
    };

...
static void add_fixed_inputs(InputSet *set) {
  ...
  static const uint32_t thresholds[] = {
      UINT32_C(0x00800000), UINT32_C(0x3e82c578), UINT32_C(0x3f0c9f54),
      UINT32_C(0x41200000),
  };
...
static void add_analytic_boundaries(InputSet *set) {
  ...
  /* Round-to-nearest transition where tanh(x) saturates at +1. */
  mpfr_set_ui(value, 1, MPFR_RNDN);
  mpfr_set_ui_2exp(scratch, 1, -25, MPFR_RNDN);
...
static void add_random_inputs(InputSet *set, size_t samples_per_stratum) {
  ...
  for (size_t i = 0; i < samples_per_stratum; ++i) {
    ...
    /* Numerically uniform inputs over the unsaturated central interval. */
    mpfr_set_ui(fraction, (unsigned long)(uint32_t)random, MPFR_RNDN);
    mpfr_div_2ui(fraction, fraction, 32, MPFR_RNDN);
    mpfr_mul_ui(value, fraction, 24, MPFR_RNDN);
    mpfr_sub_ui(value, value, 12, MPFR_RNDN);
    ...
    switch (random % 3) {
    case 0:
      mpfr_mul_d(value, fraction, 0.27, MPFR_RNDN);
      break;
    case 1:
      mpfr_mul_d(value, fraction, 0.31, MPFR_RNDN);
      mpfr_add_d(value, value, 0.24, MPFR_RNDN);
      break;
    default:
      mpfr_mul_d(value, fraction, 9.6, MPFR_RNDN);
      mpfr_add_d(value, value, 0.54, MPFR_RNDN);
      break;
    }
...
static uint32_t oracle_tanhf(uint32_t input_bits,
  ...
  if (input_bits == F32_EXPONENT_MASK ||
    ...
    *used_precision = INITIAL_ORACLE_PRECISION;
    return input_bits == F32_EXPONENT_MASK ? UINT32_C(0x3f800000)
                                           : UINT32_C(0xbf800000);
  }
...
static void emit_tests(const OracleCase *cases, size_t count,
  ...
  printf("// Code generated by tools/oracle/tanhf/generate.c; DO NOT EDIT.\n");
  printf("// Oracle: MPFR %s, tanh, binary32 round-to-nearest ties-to-even.\n",
         mpfr_get_version());
  ...
  for (size_t chunk = 0; chunk < chunk_count; ++chunk) {
    ...
    printf("///|\nfn tanhf_mpfr_cases_%zu() -> Array[(UInt, UInt)] {\n",
           chunk);
    printf("  [\n");
  ...
  printf("///|\nfn tanhf_mpfr_case_groups() -> Array[Array[(UInt, UInt)]] {\n");
  printf("  [\n");
  for (size_t chunk = 0; chunk < chunk_count; ++chunk) {
    printf("    tanhf_mpfr_cases_%zu(),\n", chunk);
  }
  printf("  ]\n}\n\n");

  printf("///|\nfn tanhf_oracle_ulp_error(expect : Float, actual : Float) -> UInt {\n");
  ...
  printf("///|\ntest \"tanhf agrees with the MPFR oracle within %u ULP\" {\n",
         options->maximum_ulp);
  printf("  let mut maximum_error = 0U\n");
  printf("  for cases in tanhf_mpfr_case_groups() {\n");
  ...
  printf("      )\n");
  printf("      let actual = @math.tanhf(input)\n");
  printf("      let error = tanhf_oracle_ulp_error(expected, actual)\n");
  printf("      if error > maximum_error {\n");
  printf("        maximum_error = error\n");
  printf("      }\n");
  printf("      if error > %uU {\n", options->maximum_ulp);
  printf("        println(\n");
  printf("          \"tanhf oracle mismatch: input_bits=\\{input_bits}, expected_bits=\\{expected_bits}, actual_bits=\\{actual.reinterpret_as_uint()}, ulp=\\{error}\",\n");
  printf("        )\n");
  ...
  printf("///|\ntest \"MoonBit Core tanhf meets the oracle bound over the corpus\" {\n");
  printf("  for cases in tanhf_mpfr_case_groups() {\n");
  printf("    for pair in cases {\n");
  printf("      let (input_bits, expected_bits) = pair\n");
  printf("      let input = Float::reinterpret_from_int(input_bits.reinterpret_as_int())\n");
  ...
  printf("        expected_bits.reinterpret_as_int(),\n");
  printf("      )\n");
  printf("      let actual = @core_math.tanhf(input)\n");
  printf("      assert_true(tanhf_oracle_ulp_error(expected, actual) <= %uU)\n",
         options->maximum_ulp);
  printf("    }\n");
  printf("  }\n");
  printf("}\n\n");

  printf("///|\ntest \"tanhf range and monotonicity over the oracle corpus\" {\n");
  ...
  printf("      assert_true(actual <= 1.0)\n");
  printf("      let opposite = @math.tanhf(-input)\n");
...
int main(int argc, char **argv) {
  ...
  for (size_t i = 0; i < inputs.length; ++i) {
    ...
    cases[i] = (OracleCase){
        ...
        .expected_bits = oracle_tanhf(inputs.items[i], &used_precision),
    };

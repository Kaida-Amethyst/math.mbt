...
static void add_analytic_boundaries(InputSet *set) {
  ...
  for (long root = -128; root <= 128; ++root) {
    mpfr_set_si(value, root, MPFR_RNDN);
    mpfr_pow_ui(value, value, 3, MPFR_RNDN);
...
static void add_random_inputs(InputSet *set, size_t samples_per_stratum) {
  ...
  for (size_t i = 0; i < samples_per_stratum; ++i) {
    ...
    /* Inputs obtained by cubing random representable candidate results. */
    uint32_t random3 = (uint32_t)xorshift64star(&state);
    uint32_t result_exponent = 77 + (random3 % 93);
    uint32_t result_bits = (random3 & F32_SIGN_MASK) |
                           (result_exponent << 23) |
                           ((uint32_t)random & F32_FRACTION_MASK);
    mpfr_set_flt(value, bits_to_float(result_bits), MPFR_RNDN);
    mpfr_pow_ui(value, value, 3, MPFR_RNDN);
...
static uint32_t oracle_cbrtf(uint32_t input_bits,
  ...
  for (mpfr_prec_t precision = INITIAL_ORACLE_PRECISION;
    ...
    mpfr_cbrt(lower, x, MPFR_RNDD);
    mpfr_cbrt(upper, x, MPFR_RNDU);
...
static void emit_tests(const OracleCase *cases, size_t count,
  ...
  printf("// Code generated by tools/oracle/cbrtf/generate.c; DO NOT EDIT.\n");
  printf("// Oracle: MPFR %s, cbrt, binary32 round-to-nearest ties-to-even.\n",
         mpfr_get_version());
  ...
  printf("// Random seed: 0x%016" PRIx64
         ...
         count);
  printf("// Inputs cover special values, every exponent boundary, exact cubes, and three deterministic random strata.\n\n");
  ...
  for (size_t chunk = 0; chunk < chunk_count; ++chunk) {
    ...
    printf("///|\nfn cbrtf_mpfr_cases_%zu() -> Array[(UInt, UInt)] {\n",
           chunk);
    printf("  [\n");
  ...
  printf("///|\nfn cbrtf_mpfr_case_groups() -> Array[Array[(UInt, UInt)]] {\n");
  printf("  [\n");
  for (size_t chunk = 0; chunk < chunk_count; ++chunk) {
    printf("    cbrtf_mpfr_cases_%zu(),\n", chunk);
  }
  printf("  ]\n}\n\n");

  printf("///|\nfn cbrtf_oracle_ulp_error(expect : Float, actual : Float) -> UInt {\n");
  ...
  printf("///|\ntest \"cbrtf agrees with the MPFR oracle within %u ULP\" {\n",
         options->maximum_ulp);
  printf("  let mut maximum_error = 0U\n");
  printf("  for cases in cbrtf_mpfr_case_groups() {\n");
  ...
  printf("      )\n");
  printf("      let actual = @math.cbrtf(input)\n");
  printf("      let error = cbrtf_oracle_ulp_error(expected, actual)\n");
  printf("      if error > maximum_error {\n");
  printf("        maximum_error = error\n");
  printf("      }\n");
  printf("      if error > %uU {\n", options->maximum_ulp);
  printf("        println(\n");
  printf("          \"cbrtf oracle mismatch: input_bits=\\{input_bits}, expected_bits=\\{expected_bits}, actual_bits=\\{actual.reinterpret_as_uint()}, ulp=\\{error}\",\n");
  printf("        )\n");
  ...
  printf("///|\ntest \"MoonBit Core cbrtf meets the oracle bound over the corpus\" {\n");
  printf("  for cases in cbrtf_mpfr_case_groups() {\n");
  printf("    for pair in cases {\n");
  printf("      let (input_bits, expected_bits) = pair\n");
  printf("      let input = Float::reinterpret_from_int(input_bits.reinterpret_as_int())\n");
  ...
  printf("        expected_bits.reinterpret_as_int(),\n");
  printf("      )\n");
  printf("      let actual = @core_math.cbrtf(input)\n");
  printf("      assert_true(cbrtf_oracle_ulp_error(expected, actual) <= %uU)\n",
         options->maximum_ulp);
  printf("    }\n");
  printf("  }\n");
  printf("}\n\n");

  printf("///|\ntest \"cbrtf monotonicity and symmetry over the oracle corpus\" {\n");
  ...
  printf("      let opposite = @math.cbrtf(-input)\n");
...
int main(int argc, char **argv) {
  ...
  for (size_t i = 0; i < inputs.length; ++i) {
    ...
    cases[i] = (OracleCase){
        ...
        .expected_bits = oracle_cbrtf(inputs.items[i], &used_precision),
    };

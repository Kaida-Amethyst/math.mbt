...
static void die(const char *message) {
  fprintf(stderr, "log10f oracle: %s\n", message);
  exit(EXIT_FAILURE);
}
...
static void add_fixed_inputs(InputSet *set) {
  static const uint32_t fixed_bits[] = {
      ...
      UINT32_C(0x05b504ec), /* observed 1 ULP witness */
...
static void add_analytic_boundaries(InputSet *set) {
  /* Every positive exponent boundary and both adjacent binade endpoints. */
  ...
  /* The exact-zero result and the argument-reduction boundary in each binade. */
  add_bit_neighborhood(set, UINT32_C(0x3f800000), 256);
  for (uint32_t exponent = 1; exponent < 255; ++exponent) {
    uint32_t boundary = (exponent << 23) | UINT32_C(0x003504f3);
    add_bit_neighborhood(set, boundary, 8);
  }

  /* Decimal powers are especially important for a base-10 logarithm. */
  for (int exponent = -45; exponent <= 38; ++exponent) {
    char decimal[16];
    snprintf(decimal, sizeof(decimal), "1e%d", exponent);
    add_bit_neighborhood(set, float_to_bits(strtof(decimal, NULL)), 8);
  }
}
...
static void add_random_inputs(InputSet *set, size_t samples_per_stratum) {
  ...
  for (size_t i = 0; i < samples_per_stratum; ++i) {
    ...
    /* Dense samples around x = 1 target cancellation in the final sum. */
    uint32_t distance = random3 & UINT32_C(0x000fffff);
    if ((random3 & UINT32_C(0x00100000)) == 0) {
      input_set_push(set, UINT32_C(0x3f800000) + distance);
    } else {
      input_set_push(set, UINT32_C(0x3f800000) - distance);
    }
  }
...
static uint32_t oracle_log10f(uint32_t input_bits,
  ...
  for (mpfr_prec_t precision = INITIAL_ORACLE_PRECISION;
    ...
    mpfr_log10(lower, x, MPFR_RNDD);
    mpfr_log10(upper, x, MPFR_RNDU);
...
static void emit_tests(const OracleCase *cases, size_t count,
  ...
  printf("// Code generated by tools/oracle/log10f/generate.c; DO NOT EDIT.\n");
  printf("// Oracle: MPFR %s, log10, binary32 round-to-nearest ties-to-even.\n",
         mpfr_get_version());
  ...
  printf("// Random seed: 0x%016" PRIx64
         ...
         count);
  printf("// Inputs cover special values, every exponent boundary, logarithmic reduction boundaries, and four deterministic random strata.\n\n");
  ...
  for (size_t chunk = 0; chunk < chunk_count; ++chunk) {
    ...
    printf("///|\nfn log10f_mpfr_cases_%zu() -> Array[(UInt, UInt)] {\n",
           chunk);
    printf("  [\n");
  ...
  printf("///|\nfn log10f_mpfr_case_groups() -> Array[Array[(UInt, UInt)]] {\n");
  printf("  [\n");
  for (size_t chunk = 0; chunk < chunk_count; ++chunk) {
    printf("    log10f_mpfr_cases_%zu(),\n", chunk);
  }
  printf("  ]\n}\n\n");

  printf("///|\nfn log10f_oracle_ulp_error(expect : Float, actual : Float) -> UInt {\n");
  ...
  printf("///|\ntest \"log10f agrees with the MPFR oracle within %u ULP\" {\n",
         options->maximum_ulp);
  printf("  let mut maximum_error = 0U\n");
  printf("  for cases in log10f_mpfr_case_groups() {\n");
  ...
  printf("      )\n");
  printf("      let actual = @math.log10f(input)\n");
  printf("      let error = log10f_oracle_ulp_error(expected, actual)\n");
  printf("      if error > maximum_error {\n");
  printf("        maximum_error = error\n");
  printf("      }\n");
  printf("      if error > %uU {\n", options->maximum_ulp);
  printf("        println(\n");
  printf("          \"log10f oracle mismatch: input_bits=\\{input_bits}, expected_bits=\\{expected_bits}, actual_bits=\\{actual.reinterpret_as_uint()}, ulp=\\{error}\",\n");
  printf("        )\n");
  ...
  printf("///|\ntest \"log10f is monotonic over its positive domain\" {\n");
...
int main(int argc, char **argv) {
  ...
  for (size_t i = 0; i < inputs.length; ++i) {
    ...
    cases[i] = (OracleCase){
        ...
        .expected_bits = oracle_log10f(inputs.items[i], &used_precision),
    };

...
static void add_fixed_inputs(InputSet *set) {
  ...
  static const uint32_t thresholds[] = {
      UINT32_C(0x39800000), UINT32_C(0x3f317217), UINT32_C(0x42b17217),
  };
...
static void add_random_inputs(InputSet *set, size_t samples_per_stratum) {
  ...
  for (size_t i = 0; i < samples_per_stratum; ++i) {
    ...
    case 0:
      ...
      break;
    case 1:
      mpfr_mul_d(value, fraction, 0.692903039935, MPFR_RNDN);
      mpfr_add_d(value, value, 0.000244140625, MPFR_RNDN);
      break;
    default:
      mpfr_mul_d(value, fraction, 89.30685281944, MPFR_RNDN);
      mpfr_add_d(value, value, 0.69314718056, MPFR_RNDN);
      break;
    }
...
static uint32_t oracle_coshf(uint32_t input_bits,
...
static void emit_tests(const OracleCase *cases, size_t count,
  ...
  printf("// Code generated by tools/oracle/coshf/generate.c; DO NOT EDIT.\n");
  printf("// Oracle: MPFR %s, cosh, binary32 round-to-nearest ties-to-even.\n",
         mpfr_get_version());
  ...
  for (size_t chunk = 0; chunk < chunk_count; ++chunk) {
    ...
    printf("///|\nfn coshf_mpfr_cases_%zu() -> Array[(UInt, UInt)] {\n",
           chunk);
    printf("  [\n");
  ...
  printf("///|\nfn coshf_mpfr_case_groups() -> Array[Array[(UInt, UInt)]] {\n");
  printf("  [\n");
  for (size_t chunk = 0; chunk < chunk_count; ++chunk) {
    printf("    coshf_mpfr_cases_%zu(),\n", chunk);
  }
  printf("  ]\n}\n\n");

  printf("///|\nfn coshf_oracle_ulp_error(expect : Float, actual : Float) -> UInt {\n");
  ...
  printf("///|\ntest \"coshf agrees with the MPFR oracle within %u ULP\" {\n",
         options->maximum_ulp);
  printf("  let mut maximum_error = 0U\n");
  printf("  for cases in coshf_mpfr_case_groups() {\n");
  ...
  printf("      )\n");
  printf("      let actual = @math.coshf(input)\n");
  printf("      let error = coshf_oracle_ulp_error(expected, actual)\n");
  printf("      if error > maximum_error {\n");
  printf("        maximum_error = error\n");
  printf("      }\n");
  printf("      if error > %uU {\n", options->maximum_ulp);
  printf("        println(\n");
  printf("          \"coshf oracle mismatch: input_bits=\\{input_bits}, expected_bits=\\{expected_bits}, actual_bits=\\{actual.reinterpret_as_uint()}, ulp=\\{error}\",\n");
  printf("        )\n");
  ...
  printf("///|\ntest \"MoonBit Core coshf meets the oracle bound over the corpus\" {\n");
  printf("  for cases in coshf_mpfr_case_groups() {\n");
  printf("    for pair in cases {\n");
  printf("      let (input_bits, expected_bits) = pair\n");
  printf("      let input = Float::reinterpret_from_int(input_bits.reinterpret_as_int())\n");
  ...
  printf("        expected_bits.reinterpret_as_int(),\n");
  printf("      )\n");
  printf("      let actual = @core_math.coshf(input)\n");
  printf("      assert_true(coshf_oracle_ulp_error(expected, actual) <= %uU)\n",
         options->maximum_ulp);
  printf("    }\n");
  printf("  }\n");
  printf("}\n\n");

  printf("///|\ntest \"coshf range and symmetry over the oracle corpus\" {\n");
  printf("  for cases in coshf_mpfr_case_groups() {\n");
  printf("    for pair in cases {\n");
  ...
  printf("      let opposite = @math.coshf(-input)\n");
...
int main(int argc, char **argv) {
  ...
  for (size_t i = 0; i < inputs.length; ++i) {
    ...
    cases[i] = (OracleCase){
        ...
        .expected_bits = oracle_coshf(inputs.items[i], &used_precision),
    };

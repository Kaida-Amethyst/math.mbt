...
static void add_fixed_inputs(InputSet *set) {
  static const uint64_t fixed[] = {
      ...
      UINT64_C(0xc06c800000000000), /* observed 1-ULP witness: -228 */
  };
...
static void add_analytic_boundaries(InputSet *set) {
  ...
  static const uint64_t transitions[] = {
      UINT64_C(0x3e40000000000000), /* 2^-27 */
      UINT64_C(0x3fdc000000000000), /* 7/16 */
      UINT64_C(0x3fe6000000000000), /* 11/16 */
      UINT64_C(0x3ff3000000000000), /* 19/16 */
      UINT64_C(0x4003800000000000), /* 39/16 */
      UINT64_C(0x4410000000000000), /* 2^66 saturation */
  };
...
static void add_random_inputs(InputSet *set, size_t samples_per_stratum) {
  ...
  static const uint64_t transitions[] = {
      UINT64_C(0x3e40000000000000), UINT64_C(0x3fdc000000000000),
      UINT64_C(0x3fe6000000000000), UINT64_C(0x3ff3000000000000),
      UINT64_C(0x4003800000000000), UINT64_C(0x4410000000000000),
  };
  for (size_t i = 0; i < samples_per_stratum; ++i) {
    ...
    input_set_push(set, (random1 & F64_SIGN_MASK) | (exponent << 52) |
                            (random2 & F64_FRACTION_MASK));

    uint64_t center = transitions[random1 % UINT64_C(6)];
    ...
    input_set_push(set, (random1 & F64_SIGN_MASK) |
                            (random0 % UINT64_C(0x3e40000000000000)));
  }
}

static uint64_t oracle_atan(uint64_t input_bits,
...
static void emit_tests(const OracleCase *cases, size_t count,
  ...
  printf("// Code generated by tools/oracle/atan/generate.c; DO NOT EDIT.\n");
  printf("// Oracle: MPFR %s, atan, binary64 round-to-nearest ties-to-even.\n",
         mpfr_get_version());
  ...
  for (size_t chunk = 0; chunk < chunk_count; ++chunk) {
    ...
    printf("///|\nfn atan_mpfr_cases_%zu() -> Array[(UInt64, UInt64)] {\n  [\n",
           chunk);
  ...
  printf("///|\nfn atan_mpfr_case_groups() -> Array[Array[(UInt64, UInt64)]] {\n  [\n");
  for (size_t chunk = 0; chunk < chunk_count; ++chunk) {
    printf("    atan_mpfr_cases_%zu(),\n", chunk);
  }
  printf("  ]\n}\n\n");

  printf("///|\nfn atan_oracle_ulp_error(expect : Double, actual : Double) -> UInt64 {\n");
  ...
  printf("///|\ntest \"atan agrees with the MPFR oracle within %u ULP\" {\n",
         options->maximum_ulp);
  printf("  let mut maximum_error = 0UL\n");
  printf("  for cases in atan_mpfr_case_groups() {\n    for pair in cases {\n");
  ...
  printf("      let actual = @math.atan(input)\n");
  printf("      let error = atan_oracle_ulp_error(expected, actual)\n");
  printf("      if error > maximum_error {\n        maximum_error = error\n      }\n");
  printf("      if error > %uUL {\n", options->maximum_ulp);
  printf("        println(\n");
  printf("          \"atan oracle mismatch: input_bits=\\{input_bits}, expected_bits=\\{expected_bits}, actual_bits=\\{actual.reinterpret_as_uint64()}, ulp=\\{error}\",\n");
  printf("        )\n");
  printf("        assert_true(false)\n      }\n    }\n  }\n");
  printf("  assert_true(maximum_error <= %uUL)\n}\n\n", options->maximum_ulp);

  printf("///|\ntest \"MoonBit Core atan meets the oracle bound over the corpus\" {\n");
  printf("  for cases in atan_mpfr_case_groups() {\n    for pair in cases {\n");
  printf("      let (input_bits, expected_bits) = pair\n");
  printf("      let expected = expected_bits.reinterpret_as_double()\n");
  printf("      let actual = @core_math.atan(input_bits.reinterpret_as_double())\n");
  printf("      assert_true(atan_oracle_ulp_error(expected, actual) <= %uUL)\n",
         options->maximum_ulp);
  printf("    }\n  }\n}\n\n");

  printf("///|\ntest \"atan is monotonic and odd over the extended real line\" {\n");
  ...
  printf("      let actual = @math.atan(input)\n");
  printf("      assert_eq(\n");
  printf("        @math.atan(-input).reinterpret_as_uint64(),\n");
...
int main(int argc, char **argv) {
  ...
  for (size_t i = 0; i < inputs.length; ++i) {
    ...
    cases[i] = (OracleCase){inputs.items[i],
                            oracle_atan(inputs.items[i], &used_precision)};

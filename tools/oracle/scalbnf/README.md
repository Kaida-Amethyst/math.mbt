...
MoonBit `scalbnf(Float, Int) -> Float` implementation.

For every finite nonzero input, the generator loads the binary32 value exactly
into MPFR, applies `mpfr_mul_2si`, and converts the exact scaled value to
binary32 with round-to-nearest, ties-to-even. Results are compared by their
complete `UInt` representation: the admitted error is zero ULP. Zero and
...
expected_bits)` tuples. It checks `@math.scalbnf` against MPFR and compares it
with MoonBit Core's `@core_math.scalbnf`. MPFR, GMP, a C compiler, `pkg-config`,
...
```sh
make -C tools/oracle/scalbnf generate
make -C tools/oracle/scalbnf verify
```

To increase each deterministic random stratum during an audit:

```sh
make -C tools/oracle/scalbnf generate SAMPLES_PER_STRATUM=32768
```
...
On 2026-08-13, the default corpus contained 15,922 unique `(input, exponent)`
...
All 101,938 resulting unique cases matched MPFR exactly in native debug and

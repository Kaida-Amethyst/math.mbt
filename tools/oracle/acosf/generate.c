...
static void add_fixed_inputs(InputSet *set) {
  static const uint32_t fixed_bits[] = {
      ...
      UINT32_C(0x32800000), /* tiny-input implementation transition */
      UINT32_C(0xb2800000),
      UINT32_C(0xbf7fde0a), /* observed 1 ULP witness */
...
static void add_random_inputs(InputSet *set, size_t samples_per_stratum) {
  ...
  for (size_t i = 0; i < samples_per_stratum; ++i) {
    ...
    static const uint32_t branch_magnitudes[] = {
        UINT32_C(0x32800000), UINT32_C(0x3f000000), UINT32_C(0x3f800000),
    };
    ...
    /* Dense samples in the tiny-input branch. */
    input_set_push(set, (random3 & F32_SIGN_MASK) |
                            (random2 % UINT32_C(0x32800000)));
  }
}

static uint32_t oracle_acosf(uint32_t input_bits,
  ...
  for (mpfr_prec_t precision = INITIAL_ORACLE_PRECISION;
    ...
    mpfr_acos(lower, x, MPFR_RNDD);
    mpfr_acos(upper, x, MPFR_RNDU);
...
static void emit_tests(const OracleCase *cases, size_t count,
  ...
  printf("// Code generated by tools/oracle/acosf/generate.c; DO NOT EDIT.\n");
  printf("// Oracle: MPFR %s, acos, binary32 round-to-nearest ties-to-even.\n",
         mpfr_get_version());
  ...
  for (size_t chunk = 0; chunk < chunk_count; ++chunk) {
    ...
    printf("///|\nfn acosf_mpfr_cases_%zu() -> Array[(UInt, UInt)] {\n",
           chunk);
    printf("  [\n");
  ...
  printf("///|\nfn acosf_mpfr_case_groups() -> Array[Array[(UInt, UInt)]] {\n");
  printf("  [\n");
  for (size_t chunk = 0; chunk < chunk_count; ++chunk) {
    printf("    acosf_mpfr_cases_%zu(),\n", chunk);
  }
  printf("  ]\n}\n\n");

  printf("///|\nfn acosf_oracle_ulp_error(expect : Float, actual : Float) -> UInt {\n");
  ...
  printf("///|\ntest \"acosf agrees with the MPFR oracle within %u ULP\" {\n",
         options->maximum_ulp);
  printf("  let mut maximum_error = 0U\n");
  printf("  for cases in acosf_mpfr_case_groups() {\n");
  ...
  printf("      )\n");
  printf("      let actual = @math.acosf(input)\n");
  printf("      let error = acosf_oracle_ulp_error(expected, actual)\n");
  printf("      if error > maximum_error {\n");
  printf("        maximum_error = error\n");
  printf("      }\n");
  printf("      if error > %uU {\n", options->maximum_ulp);
  printf("        println(\n");
  printf("          \"acosf oracle mismatch: input_bits=\\{input_bits}, expected_bits=\\{expected_bits}, actual_bits=\\{actual.reinterpret_as_uint()}, ulp=\\{error}\",\n");
  printf("        )\n");
  ...
  printf("///|\ntest \"MoonBit Core acosf meets the oracle bound over the corpus\" {\n");
  printf("  for cases in acosf_mpfr_case_groups() {\n");
  printf("    for pair in cases {\n");
  printf("      let (input_bits, expected_bits) = pair\n");
  printf("      let input = Float::reinterpret_from_int(input_bits.reinterpret_as_int())\n");
  ...
  printf("        expected_bits.reinterpret_as_int(),\n");
  printf("      )\n");
  printf("      let actual = @core_math.acosf(input)\n");
  printf("      assert_true(acosf_oracle_ulp_error(expected, actual) <= %uU)\n",
         options->maximum_ulp);
  printf("    }\n");
  printf("  }\n");
  printf("}\n\n");

  printf("///|\ntest \"acosf is decreasing and remains in range over its real domain\" {\n");
  ...
  printf("        assert_true(actual >= 0.0)\n");
  printf("        assert_true(actual <= 3.1415927410125732)\n");
  printf("        if have_previous {\n");
  printf("          if previous < actual {\n");
  printf("            println(\n");
  printf("              \"acosf monotonicity mismatch: previous_input_bits=\\{previous_input_bits}, input_bits=\\{input_bits}, previous_result_bits=\\{previous.reinterpret_as_uint()}, result_bits=\\{actual.reinterpret_as_uint()}\",\n");
  printf("            )\n");
  printf("            assert_true(false)\n");
  printf("          }\n");
  printf("        }\n");
...
int main(int argc, char **argv) {
  ...
  for (size_t i = 0; i < inputs.length; ++i) {
    ...
    cases[i] = (OracleCase){
        ...
        .expected_bits = oracle_acosf(inputs.items[i], &used_precision),
    };

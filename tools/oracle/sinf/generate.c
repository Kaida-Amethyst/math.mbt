...
static uint32_t oracle_sinf(uint32_t input_bits,
  ...
  for (mpfr_prec_t precision = INITIAL_ORACLE_PRECISION;
    ...
    mpfr_sin(lower, x, MPFR_RNDD);
    mpfr_sin(upper, x, MPFR_RNDU);
...
static void emit_tests(const OracleCase *cases, size_t count,
  ...
  printf("// Code generated by tools/oracle/sinf/generate.c; DO NOT EDIT.\n");
  printf("// Oracle: MPFR %s, sin, binary32 round-to-nearest ties-to-even.\n",
         mpfr_get_version());
  ...
  for (size_t chunk = 0; chunk < chunk_count; ++chunk) {
    ...
    printf("///|\nfn sinf_mpfr_cases_%zu() -> Array[(UInt, UInt)] {\n",
           chunk);
    printf("  [\n");
  ...
  printf("///|\nfn sinf_mpfr_case_groups() -> Array[Array[(UInt, UInt)]] {\n");
  printf("  [\n");
  for (size_t chunk = 0; chunk < chunk_count; ++chunk) {
    printf("    sinf_mpfr_cases_%zu(),\n", chunk);
  }
  printf("  ]\n}\n\n");

  printf("///|\nfn sinf_oracle_ulp_error(expect : Float, actual : Float) -> UInt {\n");
  ...
  printf("///|\ntest \"sinf agrees with the MPFR oracle within %u ULP\" {\n",
         options->maximum_ulp);
  printf("  let mut maximum_error = 0U\n");
  printf("  for cases in sinf_mpfr_case_groups() {\n");
  ...
  printf("      )\n");
  printf("      let actual = @math.sinf(input)\n");
  printf("      let error = sinf_oracle_ulp_error(expected, actual)\n");
  printf("      if error > maximum_error {\n");
  printf("        maximum_error = error\n");
  printf("      }\n");
  printf("      if error > %uU {\n", options->maximum_ulp);
  printf("        println(\n");
  printf("          \"sinf oracle mismatch: input_bits=\\{input_bits}, expected_bits=\\{expected_bits}, actual_bits=\\{actual.reinterpret_as_uint()}, ulp=\\{error}\",\n");
  printf("        )\n");
  ...
  printf("///|\ntest \"MoonBit Core sinf meets the oracle bound over the corpus\" {\n");
  printf("  for cases in sinf_mpfr_case_groups() {\n");
  printf("    for pair in cases {\n");
  printf("      let (input_bits, expected_bits) = pair\n");
  printf("      let input = Float::reinterpret_from_int(input_bits.reinterpret_as_int())\n");
  ...
  printf("        expected_bits.reinterpret_as_int(),\n");
  printf("      )\n");
  printf("      let actual = @core_math.sinf(input)\n");
  printf("      assert_true(sinf_oracle_ulp_error(expected, actual) <= %uU)\n",
         options->maximum_ulp);
  printf("    }\n");
  printf("  }\n");
  printf("}\n\n");

  printf("///|\ntest \"sinf is odd and remains in range over finite inputs\" {\n");
  printf("  for cases in sinf_mpfr_case_groups() {\n");
  printf("    for pair in cases {\n");
  ...
  printf("        let opposite = @math.sinf(-input)\n");
  printf("        assert_eq(\n");
...
int main(int argc, char **argv) {
  ...
  for (size_t i = 0; i < inputs.length; ++i) {
    ...
    cases[i] = (OracleCase){
        ...
        .expected_bits = oracle_sinf(inputs.items[i], &used_precision),
    };

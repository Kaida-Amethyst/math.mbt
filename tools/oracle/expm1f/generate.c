...
static void die(const char *message) {
  fprintf(stderr, "expm1f oracle: %s\n", message);
  exit(EXIT_FAILURE);
}
...
static void add_fixed_inputs(InputSet *set) {
  ...
  static const uint32_t thresholds[] = {
      UINT32_C(0x33000000), UINT32_C(0x3eb17218), UINT32_C(0x3f851592),
...
static void add_analytic_boundaries(InputSet *set) {
  ...
  mpfr_set_ui_2exp(value, 1, -25, MPFR_RNDN);
...
static void add_random_inputs(InputSet *set, size_t samples_per_stratum) {
  ...
  for (size_t i = 0; i < samples_per_stratum; ++i) {
    ...
    mpfr_mul_ui(value, fraction, 110, MPFR_RNDN);
    mpfr_sub_ui(value, value, 20, MPFR_RNDN);
    ...
    long k = (long)(xorshift64star(&state) % 157) - 28;
...
static uint32_t oracle_expm1f(uint32_t input_bits,
  ...
  if (input_bits == F32_EXPONENT_MASK ||
    ...
    *used_precision = INITIAL_ORACLE_PRECISION;
    return input_bits == F32_EXPONENT_MASK ? F32_EXPONENT_MASK
                                           : UINT32_C(0xbf800000);
  }
...
static void emit_tests(const OracleCase *cases, size_t count,
  ...
  printf("// Code generated by tools/oracle/expm1f/generate.c; DO NOT EDIT.\n");
  printf("// Oracle: MPFR %s, expm1, binary32 round-to-nearest ties-to-even.\n",
         mpfr_get_version());
  ...
  for (size_t chunk = 0; chunk < chunk_count; ++chunk) {
    ...
    printf("///|\nfn expm1f_mpfr_cases_%zu() -> Array[(UInt, UInt)] {\n",
           chunk);
    printf("  [\n");
  ...
  printf("///|\nfn expm1f_mpfr_case_groups() -> Array[Array[(UInt, UInt)]] {\n");
  printf("  [\n");
  for (size_t chunk = 0; chunk < chunk_count; ++chunk) {
    printf("    expm1f_mpfr_cases_%zu(),\n", chunk);
  }
  printf("  ]\n}\n\n");

  printf("///|\nfn expm1f_oracle_ulp_error(expect : Float, actual : Float) -> UInt {\n");
  ...
  printf("///|\ntest \"expm1f agrees with the MPFR oracle within %u ULP\" {\n",
         options->maximum_ulp);
  printf("  let mut maximum_error = 0U\n");
  printf("  for cases in expm1f_mpfr_case_groups() {\n");
  ...
  printf("      )\n");
  printf("      let actual = @math.expm1f(input)\n");
  printf("      let error = expm1f_oracle_ulp_error(expected, actual)\n");
  printf("      if error > maximum_error {\n");
  printf("        maximum_error = error\n");
  printf("      }\n");
  printf("      if error > %uU {\n", options->maximum_ulp);
  printf("        println(\n");
  printf("          \"expm1f oracle mismatch: input_bits=\\{input_bits}, expected_bits=\\{expected_bits}, actual_bits=\\{actual.reinterpret_as_uint()}, ulp=\\{error}\",\n");
  printf("        )\n");
  ...
  printf("///|\ntest \"MoonBit Core expm1f meets the oracle bound over the corpus\" {\n");
  printf("  for cases in expm1f_mpfr_case_groups() {\n");
  printf("    for pair in cases {\n");
  printf("      let (input_bits, expected_bits) = pair\n");
  printf("      let input = Float::reinterpret_from_int(input_bits.reinterpret_as_int())\n");
  ...
  printf("        expected_bits.reinterpret_as_int(),\n");
  printf("      )\n");
  printf("      let actual = @core_math.expm1f(input)\n");
  printf("      assert_true(expm1f_oracle_ulp_error(expected, actual) <= %uU)\n",
         options->maximum_ulp);
  printf("    }\n");
  printf("  }\n");
  printf("}\n\n");

  printf("///|\ntest \"expm1f range and monotonicity over the oracle corpus\" {\n");
...
int main(int argc, char **argv) {
  ...
  for (size_t i = 0; i < inputs.length; ++i) {
    ...
    cases[i] = (OracleCase){
        ...
        .expected_bits = oracle_expm1f(inputs.items[i], &used_precision),
    };

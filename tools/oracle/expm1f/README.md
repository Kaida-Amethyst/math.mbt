...
MoonBit `expm1f(Float) -> Float` implementation.
...
```sh
make -C tools/oracle/expm1f generate
make -C tools/oracle/expm1f verify
```

For a larger audit that searches for incorrectly rounded results:

```sh
make -C tools/oracle/expm1f generate SAMPLES_PER_STRATUM=32768 MAX_ULP=0
```
...
On 2026-08-13, the default corpus contained 15,541 unique inputs generated with
...
producing 101,481 unique inputs. It passed in native debug and release mode.
The fixed input `1.0` demonstrates the observed one-ULP error:

```text
input bits    = 0x3f800000
MPFR result   = 0x3fdbf0a9
actual result = 0x3fdbf0a8
error         = 1 ULP
```

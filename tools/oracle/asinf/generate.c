...
static void add_fixed_inputs(InputSet *set) {
  static const uint32_t fixed_bits[] = {
      ...
      UINT32_C(0xbf30c1e8), /* observed 1 ULP witness */
...
static void add_random_inputs(InputSet *set, size_t samples_per_stratum) {
  ...
  for (size_t i = 0; i < samples_per_stratum; ++i) {
    ...
    static const uint32_t branch_magnitudes[] = {
        UINT32_C(0x39800000), UINT32_C(0x3f000000), UINT32_C(0x3f800000),
    };
...
static uint32_t oracle_asinf(uint32_t input_bits,
...
static void emit_tests(const OracleCase *cases, size_t count,
  ...
  printf("// Code generated by tools/oracle/asinf/generate.c; DO NOT EDIT.\n");
  printf("// Oracle: MPFR %s, asin, binary32 round-to-nearest ties-to-even.\n",
         mpfr_get_version());
  ...
  for (size_t chunk = 0; chunk < chunk_count; ++chunk) {
    ...
    printf("///|\nfn asinf_mpfr_cases_%zu() -> Array[(UInt, UInt)] {\n",
           chunk);
    printf("  [\n");
  ...
  printf("///|\nfn asinf_mpfr_case_groups() -> Array[Array[(UInt, UInt)]] {\n");
  printf("  [\n");
  for (size_t chunk = 0; chunk < chunk_count; ++chunk) {
    printf("    asinf_mpfr_cases_%zu(),\n", chunk);
  }
  printf("  ]\n}\n\n");

  printf("///|\nfn asinf_oracle_ulp_error(expect : Float, actual : Float) -> UInt {\n");
  ...
  printf("///|\ntest \"asinf agrees with the MPFR oracle within %u ULP\" {\n",
         options->maximum_ulp);
  printf("  let mut maximum_error = 0U\n");
  printf("  for cases in asinf_mpfr_case_groups() {\n");
  ...
  printf("      )\n");
  printf("      let actual = @math.asinf(input)\n");
  printf("      let error = asinf_oracle_ulp_error(expected, actual)\n");
  printf("      if error > maximum_error {\n");
  printf("        maximum_error = error\n");
  printf("      }\n");
  printf("      if error > %uU {\n", options->maximum_ulp);
  printf("        println(\n");
  printf("          \"asinf oracle mismatch: input_bits=\\{input_bits}, expected_bits=\\{expected_bits}, actual_bits=\\{actual.reinterpret_as_uint()}, ulp=\\{error}\",\n");
  printf("        )\n");
  ...
  printf("///|\ntest \"MoonBit Core asinf meets the oracle bound over the corpus\" {\n");
  printf("  for cases in asinf_mpfr_case_groups() {\n");
  printf("    for pair in cases {\n");
  printf("      let (input_bits, expected_bits) = pair\n");
  printf("      let input = Float::reinterpret_from_int(input_bits.reinterpret_as_int())\n");
  ...
  printf("        expected_bits.reinterpret_as_int(),\n");
  printf("      )\n");
  printf("      let actual = @core_math.asinf(input)\n");
  printf("      assert_true(asinf_oracle_ulp_error(expected, actual) <= %uU)\n",
         options->maximum_ulp);
  printf("    }\n");
  printf("  }\n");
  printf("}\n\n");

  printf("///|\ntest \"asinf is monotonic and odd over its real domain\" {\n");
  ...
  printf("        let opposite = @math.asinf(-input)\n");
  printf("        assert_eq(\n");
  ...
  printf("            println(\n");
  printf("              \"asinf monotonicity mismatch: previous_input_bits=\\{previous_input_bits}, input_bits=\\{input_bits}, previous_result_bits=\\{previous.reinterpret_as_uint()}, result_bits=\\{actual.reinterpret_as_uint()}\",\n");
  printf("            )\n");
  printf("            assert_true(false)\n");
  printf("          }\n");
  printf("        }\n");
...
int main(int argc, char **argv) {
  ...
  for (size_t i = 0; i < inputs.length; ++i) {
    ...
    cases[i] = (OracleCase){
        ...
        .expected_bits = oracle_asinf(inputs.items[i], &used_precision),
    };

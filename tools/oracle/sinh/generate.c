...
static void add_fixed_inputs(InputSet *set) {
  static const uint64_t fixed_bits[] = {
      ...
      UINT64_C(0xbfeabc3c7931e986), /* observed two-ULP witness */
...
static void add_random_inputs(InputSet *set, size_t samples_per_stratum) {
  ...
  for (size_t i = 0; i < samples_per_stratum; ++i) {
    ...
    case 1:
      ...
      break;
    case 2:
      mpfr_mul_ui(value, fraction, 21, MPFR_RNDN);
...
static uint64_t oracle_sinh(uint64_t input_bits,
...
static void emit_tests(const OracleCase *cases, size_t count,
  ...
  printf("// Code generated by tools/oracle/sinh/generate.c; DO NOT EDIT.\n");
  printf("// Oracle: MPFR %s, sinh, binary64 round-to-nearest ties-to-even.\n",
         mpfr_get_version());
  ...
  for (size_t chunk = 0; chunk < chunk_count; ++chunk) {
    ...
    printf("///|\nfn sinh_mpfr_cases_%zu() -> Array[(UInt64, UInt64)] {\n",
           chunk);
    printf("  [\n");
  ...
  printf("///|\nfn sinh_mpfr_case_groups() -> Array[Array[(UInt64, UInt64)]] {\n");
  printf("  [\n");
  for (size_t chunk = 0; chunk < chunk_count; ++chunk) {
    printf("    sinh_mpfr_cases_%zu(),\n", chunk);
  }
  printf("  ]\n}\n\n");

  printf("///|\nfn sinh_oracle_ulp_error(expect : Double, actual : Double) -> UInt64 {\n");
  ...
  printf("///|\ntest \"sinh agrees with the MPFR oracle within %u ULP\" {\n",
         options->maximum_ulp);
  printf("  let mut maximum_error = 0UL\n");
  printf("  for cases in sinh_mpfr_case_groups() {\n");
  ...
  printf("      let actual = @math.sinh(input)\n");
  printf("      let error = sinh_oracle_ulp_error(expected, actual)\n");
  printf("      if error > maximum_error {\n");
  printf("        maximum_error = error\n");
  printf("      }\n");
  printf("      if error > %uUL {\n", options->maximum_ulp);
  printf("        println(\n");
  printf("          \"sinh oracle mismatch: input_bits=\\{input_bits}, expected_bits=\\{expected_bits}, actual_bits=\\{actual.reinterpret_as_uint64()}, ulp=\\{error}\",\n");
  printf("        )\n");
  ...
  printf("///|\ntest \"MoonBit Core sinh stays within %d ULP over the corpus\" {\n",
         CORE_MAXIMUM_ULP);
  printf("  for cases in sinh_mpfr_case_groups() {\n");
  printf("    for pair in cases {\n");
  printf("      let (input_bits, expected_bits) = pair\n");
  printf("      let input = input_bits.reinterpret_as_double()\n");
  printf("      let expected = expected_bits.reinterpret_as_double()\n");
  printf("      let actual = @core_math.sinh(input)\n");
  printf("      let error = sinh_oracle_ulp_error(expected, actual)\n");
  printf("      if error > %dUL {\n", CORE_MAXIMUM_ULP);
  printf("        println(\n");
  printf("          \"Core sinh oracle mismatch: input_bits=\\{input_bits}, expected_bits=\\{expected_bits}, actual_bits=\\{actual.reinterpret_as_uint64()}, ulp=\\{error}\",\n");
  printf("        )\n");
  printf("        assert_true(false)\n");
  printf("      }\n");
  printf("    }\n");
  ...
  printf("///|\ntest \"sinh odd symmetry and monotonicity over the oracle corpus\" {\n");
  ...
  printf("      let opposite = @math.sinh(-input)\n");
  printf("      assert_eq(\n");
...
int main(int argc, char **argv) {
  ...
  for (size_t i = 0; i < inputs.length; ++i) {
    ...
    cases[i] = (OracleCase){
        ...
        .expected_bits = oracle_sinh(inputs.items[i], &used_precision),
    };

...
static void add_fixed_inputs(InputSet *set) {
  static const uint64_t fixed[] = {
      ...
      UINT64_C(0xbfe0b994d4cde728), /* observed 1-ULP witness */
  };
...
static void add_analytic_boundaries(InputSet *set) {
  ...
  static const uint64_t transitions[] = {
      UINT64_C(0x3c60000000000000), /* high word at the tiny shortcut */
      UINT64_C(0x3c60000100000000), /* first high word after the shortcut */
...
static void add_random_inputs(InputSet *set, size_t samples_per_stratum) {
  ...
  static const uint64_t transitions[] = {
      UINT64_C(0x3c60000000000000), UINT64_C(0x3c60000100000000),
  ...
  for (size_t i = 0; i < samples_per_stratum; ++i) {
    ...
    input_set_push(set, (random1 & F64_SIGN_MASK) |
                            (random0 % UINT64_C(0x3c60000100000000)));
  }
}

static uint64_t oracle_acos(uint64_t input_bits,
...
static void emit_tests(const OracleCase *cases, size_t count,
  ...
  printf("// Code generated by tools/oracle/acos/generate.c; DO NOT EDIT.\n");
  printf("// Oracle: MPFR %s, acos, binary64 round-to-nearest ties-to-even.\n",
         mpfr_get_version());
  ...
  for (size_t chunk = 0; chunk < chunk_count; ++chunk) {
    ...
    printf("///|\nfn acos_mpfr_cases_%zu() -> Array[(UInt64, UInt64)] {\n  [\n",
           chunk);
  ...
  printf("///|\nfn acos_mpfr_case_groups() -> Array[Array[(UInt64, UInt64)]] {\n  [\n");
  for (size_t chunk = 0; chunk < chunk_count; ++chunk) {
    printf("    acos_mpfr_cases_%zu(),\n", chunk);
  }
  printf("  ]\n}\n\n");

  printf("///|\nfn acos_oracle_ulp_error(expect : Double, actual : Double) -> UInt64 {\n");
  ...
  printf("///|\ntest \"acos agrees with the MPFR oracle within %u ULP\" {\n",
         options->maximum_ulp);
  printf("  let mut maximum_error = 0UL\n");
  printf("  for cases in acos_mpfr_case_groups() {\n    for pair in cases {\n");
  ...
  printf("      let actual = @math.acos(input)\n");
  printf("      let error = acos_oracle_ulp_error(expected, actual)\n");
  printf("      if error > maximum_error {\n        maximum_error = error\n      }\n");
  printf("      if error > %uUL {\n", options->maximum_ulp);
  printf("        println(\n");
  printf("          \"acos oracle mismatch: input_bits=\\{input_bits}, expected_bits=\\{expected_bits}, actual_bits=\\{actual.reinterpret_as_uint64()}, ulp=\\{error}\",\n");
  printf("        )\n");
  printf("        assert_true(false)\n      }\n    }\n  }\n");
  printf("  assert_true(maximum_error <= %uUL)\n}\n\n", options->maximum_ulp);

  printf("///|\ntest \"MoonBit Core acos meets the oracle bound over the corpus\" {\n");
  printf("  for cases in acos_mpfr_case_groups() {\n    for pair in cases {\n");
  printf("      let (input_bits, expected_bits) = pair\n");
  printf("      let expected = expected_bits.reinterpret_as_double()\n");
  printf("      let actual = @core_math.acos(input_bits.reinterpret_as_double())\n");
  printf("      assert_true(acos_oracle_ulp_error(expected, actual) <= %uUL)\n",
         options->maximum_ulp);
  printf("    }\n  }\n}\n\n");

  printf("///|\ntest \"acos is decreasing and stays in range over its real domain\" {\n");
  ...
  printf("        let actual = @math.acos(input)\n");
  printf("        assert_true(actual >= 0.0 && actual <= 3.1415926535897931160)\n");
...
int main(int argc, char **argv) {
  ...
  for (size_t i = 0; i < inputs.length; ++i) {
    ...
    cases[i] = (OracleCase){inputs.items[i],
                            oracle_acos(inputs.items[i], &used_precision)};

...
static void add_fixed_inputs(InputSet *set) {
  static const uint32_t fixed_bits[] = {
      ...
      UINT32_C(0xc3878000), /* observed 1 ULP witness */
...
static void add_analytic_boundaries(InputSet *set) {
  /* Every signed exponent boundary and both adjacent binade endpoints. */
  ...
  static const uint32_t branch_magnitudes[] = {
      ...
      UINT32_C(0x39800000), /* 2^-12, tiny passthrough */
      UINT32_C(0x3ee00000), /* 7/16 */
      UINT32_C(0x3f300000), /* 11/16 */
      UINT32_C(0x3f980000), /* 19/16 */
      UINT32_C(0x401c0000), /* 39/16 */
      UINT32_C(0x4c800000), /* large-input saturation */
  };
  for (size_t i = 0;
...
static void add_random_inputs(InputSet *set, size_t samples_per_stratum) {
  ...
  for (size_t i = 0; i < samples_per_stratum; ++i) {
    ...
    /* A second signed exponent stream diversifies mantissas in every binade. */
    ...
    /* Dense samples around one of the six nonzero branch boundaries. */
    static const uint32_t branch_magnitudes[] = {
        UINT32_C(0x39800000), UINT32_C(0x3ee00000),
        UINT32_C(0x3f300000), UINT32_C(0x3f980000),
        UINT32_C(0x401c0000), UINT32_C(0x4c800000),
    };
    uint32_t center = branch_magnitudes[random3 % UINT32_C(6)];
...
static uint32_t oracle_atanf(uint32_t input_bits,
  ...
  for (mpfr_prec_t precision = INITIAL_ORACLE_PRECISION;
    ...
    mpfr_atan(lower, x, MPFR_RNDD);
    mpfr_atan(upper, x, MPFR_RNDU);
...
static void emit_tests(const OracleCase *cases, size_t count,
  ...
  printf("// Code generated by tools/oracle/atanf/generate.c; DO NOT EDIT.\n");
  printf("// Oracle: MPFR %s, atan, binary32 round-to-nearest ties-to-even.\n",
         mpfr_get_version());
  ...
  printf("// Random seed: 0x%016" PRIx64
         ...
         count);
  printf("// Inputs cover special values, every signed exponent boundary, all implementation transitions, and four deterministic random strata.\n\n");
  ...
  for (size_t chunk = 0; chunk < chunk_count; ++chunk) {
    ...
    printf("///|\nfn atanf_mpfr_cases_%zu() -> Array[(UInt, UInt)] {\n",
           chunk);
    printf("  [\n");
  ...
  printf("///|\nfn atanf_mpfr_case_groups() -> Array[Array[(UInt, UInt)]] {\n");
  printf("  [\n");
  for (size_t chunk = 0; chunk < chunk_count; ++chunk) {
    printf("    atanf_mpfr_cases_%zu(),\n", chunk);
  }
  printf("  ]\n}\n\n");

  printf("///|\nfn atanf_oracle_ulp_error(expect : Float, actual : Float) -> UInt {\n");
  ...
  printf("///|\ntest \"atanf agrees with the MPFR oracle within %u ULP\" {\n",
         options->maximum_ulp);
  printf("  let mut maximum_error = 0U\n");
  printf("  for cases in atanf_mpfr_case_groups() {\n");
  ...
  printf("      )\n");
  printf("      let actual = @math.atanf(input)\n");
  printf("      let error = atanf_oracle_ulp_error(expected, actual)\n");
  printf("      if error > maximum_error {\n");
  printf("        maximum_error = error\n");
  printf("      }\n");
  printf("      if error > %uU {\n", options->maximum_ulp);
  printf("        println(\n");
  printf("          \"atanf oracle mismatch: input_bits=\\{input_bits}, expected_bits=\\{expected_bits}, actual_bits=\\{actual.reinterpret_as_uint()}, ulp=\\{error}\",\n");
  printf("        )\n");
  ...
  printf("///|\ntest \"MoonBit Core atanf meets the oracle bound over the corpus\" {\n");
  printf("  for cases in atanf_mpfr_case_groups() {\n");
  printf("    for pair in cases {\n");
  printf("      let (input_bits, expected_bits) = pair\n");
  printf("      let input = Float::reinterpret_from_int(input_bits.reinterpret_as_int())\n");
  ...
  printf("        expected_bits.reinterpret_as_int(),\n");
  printf("      )\n");
  printf("      let actual = @core_math.atanf(input)\n");
  printf("      assert_true(atanf_oracle_ulp_error(expected, actual) <= %uU)\n",
         options->maximum_ulp);
  printf("    }\n");
  printf("  }\n");
  printf("}\n\n");

  printf("///|\ntest \"atanf is monotonic and odd over the extended real line\" {\n");
  printf("  let mut previous : Float = 0.0\n");
  printf("  let mut previous_input_bits = 0U\n");
  ...
  printf("      let opposite = @math.atanf(-input)\n");
  printf("      assert_eq(opposite.reinterpret_as_uint(), (-actual).reinterpret_as_uint())\n");
  printf("      if have_previous {\n");
  printf("        if previous > actual {\n");
  printf("          println(\n");
  printf("            \"atanf monotonicity mismatch: previous_input_bits=\\{previous_input_bits}, input_bits=\\{input_bits}, previous_result_bits=\\{previous.reinterpret_as_uint()}, result_bits=\\{actual.reinterpret_as_uint()}\",\n");
  printf("          )\n");
  printf("          assert_true(false)\n");
  printf("        }\n");
  printf("      }\n");
  printf("      previous = actual\n");
  printf("      previous_input_bits = input_bits\n");
...
int main(int argc, char **argv) {
  ...
  for (size_t i = 0; i < inputs.length; ++i) {
    ...
    cases[i] = (OracleCase){
        ...
        .expected_bits = oracle_atanf(inputs.items[i], &used_precision),
    };

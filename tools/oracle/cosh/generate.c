...
static void add_fixed_inputs(InputSet *set) {
  static const uint64_t fixed_bits[] = {
      ...
      UINT64_C(0xc08633ce8fb9f87d), /* observed one-ULP witness */
  ...
  for (int value = -720; value <= 720; ++value) {
  ...
  static const uint64_t thresholds[] = {
      UINT64_C(0x3c80000000000000), UINT64_C(0x3fd62e4300000000),
      UINT64_C(0x4036000000000000), UINT64_C(0x40862e4200000000),
      UINT64_C(0x408633ce8fb9f87d),
  };
...
static void add_random_inputs(InputSet *set, size_t samples_per_stratum) {
  ...
  for (size_t i = 0; i < samples_per_stratum; ++i) {
    ...
    mpfr_mul_ui(value, fraction, 1440, MPFR_RNDN);
    mpfr_sub_ui(value, value, 720, MPFR_RNDN);
    ...
    switch (random % 5) {
    case 0:
      mpfr_mul_2si(value, fraction, -55, MPFR_RNDN);
      break;
    case 1:
      mpfr_mul_d(value, fraction, 0.34657359027997263, MPFR_RNDN);
      break;
    case 2:
      mpfr_mul_d(value, fraction, 21.653426409720027, MPFR_RNDN);
      mpfr_add_d(value, value, 0.34657359027997263, MPFR_RNDN);
      break;
    case 3:
      mpfr_mul_d(value, fraction, 687.782712893384, MPFR_RNDN);
      mpfr_add_ui(value, value, 22, MPFR_RNDN);
      break;
    default:
      mpfr_mul_d(value, fraction, 1.0, MPFR_RNDN);
      mpfr_add_d(value, value, 709.5, MPFR_RNDN);
      break;
    }
...
static uint64_t oracle_cosh(uint64_t input_bits,
  ...
  for (mpfr_prec_t precision = INITIAL_ORACLE_PRECISION;
    ...
    mpfr_cosh(lower, x, MPFR_RNDD);
    mpfr_cosh(upper, x, MPFR_RNDU);
...
static void emit_tests(const OracleCase *cases, size_t count,
  ...
  printf("// Code generated by tools/oracle/cosh/generate.c; DO NOT EDIT.\n");
  printf("// Oracle: MPFR %s, cosh, binary64 round-to-nearest ties-to-even.\n",
         mpfr_get_version());
  ...
  for (size_t chunk = 0; chunk < chunk_count; ++chunk) {
    ...
    printf("///|\nfn cosh_mpfr_cases_%zu() -> Array[(UInt64, UInt64)] {\n",
           chunk);
    printf("  [\n");
  ...
  printf("///|\nfn cosh_mpfr_case_groups() -> Array[Array[(UInt64, UInt64)]] {\n");
  printf("  [\n");
  for (size_t chunk = 0; chunk < chunk_count; ++chunk) {
    printf("    cosh_mpfr_cases_%zu(),\n", chunk);
  }
  printf("  ]\n}\n\n");

  printf("///|\nfn cosh_oracle_ulp_error(expect : Double, actual : Double) -> UInt64 {\n");
  ...
  printf("///|\ntest \"cosh agrees with the MPFR oracle within %u ULP\" {\n",
         options->maximum_ulp);
  printf("  let mut maximum_error = 0UL\n");
  printf("  for cases in cosh_mpfr_case_groups() {\n");
  ...
  printf("      let actual = @math.cosh(input)\n");
  printf("      let error = cosh_oracle_ulp_error(expected, actual)\n");
  printf("      if error > maximum_error {\n");
  printf("        maximum_error = error\n");
  printf("      }\n");
  printf("      if error > %uUL {\n", options->maximum_ulp);
  printf("        println(\n");
  printf("          \"cosh oracle mismatch: input_bits=\\{input_bits}, expected_bits=\\{expected_bits}, actual_bits=\\{actual.reinterpret_as_uint64()}, ulp=\\{error}\",\n");
  printf("        )\n");
  ...
  printf("///|\ntest \"MoonBit Core cosh stays within %d ULP over the corpus\" {\n",
         CORE_MAXIMUM_ULP);
  printf("  for cases in cosh_mpfr_case_groups() {\n");
  printf("    for pair in cases {\n");
  printf("      let (input_bits, expected_bits) = pair\n");
  printf("      let input = input_bits.reinterpret_as_double()\n");
  printf("      let expected = expected_bits.reinterpret_as_double()\n");
  printf("      let actual = @core_math.cosh(input)\n");
  printf("      let error = cosh_oracle_ulp_error(expected, actual)\n");
  printf("      if error > %dUL {\n", CORE_MAXIMUM_ULP);
  printf("        println(\n");
  printf("          \"Core cosh oracle mismatch: input_bits=\\{input_bits}, expected_bits=\\{expected_bits}, actual_bits=\\{actual.reinterpret_as_uint64()}, ulp=\\{error}\",\n");
  printf("        )\n");
  printf("        assert_true(false)\n");
  printf("      }\n");
  printf("    }\n");
  ...
  printf("///|\ntest \"cosh even symmetry and range over the oracle corpus\" {\n");
  printf("  for cases in cosh_mpfr_case_groups() {\n");
  printf("    for pair in cases {\n");
  ...
  printf("      let opposite = @math.cosh(-input)\n");
  printf("      assert_eq(\n");
...
int main(int argc, char **argv) {
  ...
  for (size_t i = 0; i < inputs.length; ++i) {
    ...
    cases[i] = (OracleCase){
        ...
        .expected_bits = oracle_cosh(inputs.items[i], &used_precision),
    };

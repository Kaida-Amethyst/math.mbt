...
static void die(const char *message) {
  fprintf(stderr, "acoshf oracle: %s\n", message);
  exit(EXIT_FAILURE);
}
...
static void add_fixed_inputs(InputSet *set) {
  static const uint32_t fixed_bits[] = {
      ...
      UINT32_C(0x3f803e90), /* observed 2 ULP witness */
...
static void add_analytic_boundaries(InputSet *set) {
  /* Every exponent boundary and both adjacent binade endpoints. */
...
static void add_random_inputs(InputSet *set, size_t samples_per_stratum) {
  ...
  for (size_t i = 0; i < samples_per_stratum; ++i) {
    ...
    /* A valid-domain exponent-stratified stream covers [1, +infinity). */
    uint32_t random3 = (uint32_t)xorshift64star(&state);
    uint32_t exponent2 = UINT32_C(0x7f) + random3 % UINT32_C(0x80);
    input_set_push(set, (exponent2 << 23) |
                            ((uint32_t)random & F32_FRACTION_MASK));

    /* Dense samples around one of the three nonzero branch boundaries. */
    static const uint32_t branch_magnitudes[] = {
        UINT32_C(0x3f800000), UINT32_C(0x40000000), UINT32_C(0x45800000),
    };
    ...
    /* Dense samples on both sides of x = 1 target cancellation and domain. */
    uint32_t near_one = random2 & UINT32_C(0x001fffff);
    input_set_push(set, UINT32_C(0x3f800000) + near_one);
    input_set_push(set, UINT32_C(0x3f800000) - near_one);
  }
}

static uint32_t oracle_acoshf(uint32_t input_bits,
  ...
  for (mpfr_prec_t precision = INITIAL_ORACLE_PRECISION;
    ...
    mpfr_acosh(lower, x, MPFR_RNDD);
    mpfr_acosh(upper, x, MPFR_RNDU);
...
static void emit_tests(const OracleCase *cases, size_t count,
  ...
  printf("// Code generated by tools/oracle/acoshf/generate.c; DO NOT EDIT.\n");
  printf("// Oracle: MPFR %s, acosh, binary32 round-to-nearest ties-to-even.\n",
         mpfr_get_version());
  ...
  printf("// Random seed: 0x%016" PRIx64
         ...
         count);
  printf("// Inputs cover special values, every exponent and implementation branch boundary, and four deterministic random strata.\n\n");
  ...
  for (size_t chunk = 0; chunk < chunk_count; ++chunk) {
    ...
    printf("///|\nfn acoshf_mpfr_cases_%zu() -> Array[(UInt, UInt)] {\n",
           chunk);
    printf("  [\n");
  ...
  printf("///|\nfn acoshf_mpfr_case_groups() -> Array[Array[(UInt, UInt)]] {\n");
  printf("  [\n");
  for (size_t chunk = 0; chunk < chunk_count; ++chunk) {
    printf("    acoshf_mpfr_cases_%zu(),\n", chunk);
  }
  printf("  ]\n}\n\n");

  printf("///|\nfn acoshf_oracle_ulp_error(expect : Float, actual : Float) -> UInt {\n");
  ...
  printf("///|\ntest \"acoshf agrees with the MPFR oracle within %u ULP\" {\n",
         options->maximum_ulp);
  printf("  let mut maximum_error = 0U\n");
  printf("  for cases in acoshf_mpfr_case_groups() {\n");
  ...
  printf("      )\n");
  printf("      let actual = @math.acoshf(input)\n");
  printf("      let error = acoshf_oracle_ulp_error(expected, actual)\n");
  printf("      if error > maximum_error {\n");
  printf("        maximum_error = error\n");
  printf("      }\n");
  printf("      if error > %uU {\n", options->maximum_ulp);
  printf("        println(\n");
  printf("          \"acoshf oracle mismatch: input_bits=\\{input_bits}, expected_bits=\\{expected_bits}, actual_bits=\\{actual.reinterpret_as_uint()}, ulp=\\{error}\",\n");
  printf("        )\n");
  ...
  printf("///|\ntest \"MoonBit Core acoshf meets the oracle bound on its unaffected range\" {\n");
  printf("  for cases in acoshf_mpfr_case_groups() {\n");
  printf("    for pair in cases {\n");
  printf("      let (input_bits, expected_bits) = pair\n");
  printf("      let input = Float::reinterpret_from_int(input_bits.reinterpret_as_int())\n");
  ...
  printf("        expected_bits.reinterpret_as_int(),\n");
  printf("      )\n");
  printf("      if input >= 1.0 && input < 4096.0 {\n");
  printf("        let actual = @core_math.acoshf(input)\n");
  printf("        assert_true(acoshf_oracle_ulp_error(expected, actual) <= %uU)\n",
         options->maximum_ulp);
  printf("      }\n");
  printf("    }\n");
  printf("  }\n");
  ...
  printf("///|\ntest \"acoshf is monotonic over its real domain\" {\n");
...
int main(int argc, char **argv) {
  ...
  for (size_t i = 0; i < inputs.length; ++i) {
    ...
    cases[i] = (OracleCase){
        ...
        .expected_bits = oracle_acoshf(inputs.items[i], &used_precision),
    };
